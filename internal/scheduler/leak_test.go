...
func TestOpenScheduler_ZeroGoroutineLeak(t *testing.T) {
	ts := testtarget.NewServer()
	defer ts.Close()
	...
	p := createOpenTestPlan(ts.URL(), 100, time.Second, 20, 300*time.Millisecond, 500*time.Millisecond)
	exec, err := executor.NewHTTPExecutor(p)
	if err != nil {
		t.Fatalf("failed to create http executor: %v", err)
	}
	...
	if finalGoroutines > initialGoroutines {
		t.Errorf("goroutine leak detected in open scheduler: initial %d, final %d (+%d)",
			initialGoroutines, finalGoroutines, finalGoroutines-initialGoroutines)
	}
}

func TestOpenScheduler_ZeroConnectionLeak(t *testing.T) {
	ts := testtarget.NewServer()
	defer ts.Close()
...
func TestOpenScheduler_BoundedMemorySoak(t *testing.T) {
	ts := testtarget.NewServer()
	defer ts.Close()

	p := createOpenTestPlan(ts.URL(), 200, time.Second, 30, 500*time.Millisecond, 500*time.Millisecond)
	exec, err := executor.NewHTTPExecutor(p)
	if err != nil {
		t.Fatalf("failed to create http executor: %v", err)
	}
	...
	// Verify heap allocation is bounded (Alloc < 50MB)
	if m2.Alloc > 50*1024*1024 {
		t.Errorf("excessive memory allocation during open soak: %d bytes", m2.Alloc)
	}
}
